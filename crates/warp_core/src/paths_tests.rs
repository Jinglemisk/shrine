...
fn test_data_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(data_dir(), home_dir.join(".shrine"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(data_dir(), home_dir.join(".local/share/Shrine"));
        } else if #[cfg(windows)] {
            assert_eq!(data_dir(), home_dir.join("AppData\\Roaming\\shrine\\Shrine\\data"));
        } else {
...
fn test_config_local_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(config_local_dir(), home_dir.join(".shrine"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(config_local_dir(), home_dir.join(".config/Shrine"));
        } else if #[cfg(windows)] {
            assert_eq!(config_local_dir(), home_dir.join("AppData\\Local\\shrine\\Shrine\\config"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_warp_home_config_dir_path() {
    ...
    let expected_dir_name = match ChannelState::data_profile() {
        Some(data_profile) => format!(".shrine-{data_profile}"),
        None => ".shrine".to_string(),
    };
...
fn test_cache_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(cache_dir(), home_dir.join("Library/Application Support/dev.shrine.Shrine"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(cache_dir(), home_dir.join(".cache/Shrine"));
        } else if #[cfg(windows)] {
            assert_eq!(cache_dir(), home_dir.join("AppData\\Local\\shrine\\Shrine\\cache"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_state_dir_path() {
    ...
    cfg_if::cfg_if! {
        // ChannelState, by default, is configured for Channel::Oss.
        if #[cfg(target_os = "macos")] {
            assert_eq!(state_dir(), home_dir.join("Library/Application Support/dev.shrine.Shrine"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(state_dir(), home_dir.join(".local/state/Shrine"));
        } else if #[cfg(windows)] {
            assert_eq!(state_dir(), home_dir.join("AppData\\Local\\shrine\\Shrine\\data"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_project_path_for_oss_app_id() {
    let project_dirs = project_dirs_for_app_id(AppId::new("dev", "shrine", "Shrine"), None)
        .expect("should be able to compute project dirs");
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "dev.shrine.Shrine");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(project_dirs.project_path(), "Shrine");
        } else if #[cfg(windows)] {
            assert_eq!(project_dirs.project_path(), "shrine\\Shrine");
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
